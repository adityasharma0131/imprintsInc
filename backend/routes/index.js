...
router.get("/api/category/:name", async (req, res) => {
  const categoryName = req.params.name; // Get the category name from the request
  try {
    const category = await CategoryModel.findOne({ name: categoryName }); // Find category by name
    if (!category) {
      return res.status(404).json({ message: "Category not found" });
    }
    res.status(200).json(category);
